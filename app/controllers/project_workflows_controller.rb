class ProjectWorkflowsController < WorkflowsController
  ...
  before_filter :find_trackers_roles_and_statuses_for_edit, only: [:edit,
                                                                   :save,
                                                                   :permissions,
                                                                   :save_permissions]
  ...
  def permissions
    ...
    @fields = (Tracker::CORE_FIELDS_ALL - @trackers
      .map(&:disabled_core_fields).reduce(:&))
              .map { |field| [field, l('field_' + field.sub(/_id$/, ''))] }
  ...
  def save_permissions
    ...
    redirect_to edit_project_workflows_permissions_path(role_id: @roles, tracker_id: @trackers)
  end
  ...
  def create_project_workflows
    workflows = []
    WorkflowTransition
      .where(role_id: @roles.map(&:id), tracker_id: @trackers
        .map(&:id)).each do |workflow|
